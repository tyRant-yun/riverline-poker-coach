...
- 每个新任务、新 Agent 进程或上下文压缩后，必须先完整读取本文件；首次用户可见回复的第一句必须**严格为**：`Yes Sir! AGENTS.md loaded.`
- 同一任务和同一未压缩上下文中的后续用户可见回复，第一句只需**严格为**：`Yes Sir!`
- 只有实际完整读取成功后才能声称 `AGENTS.md loaded.`；读取失败必须明确报告，不能伪造确认。
- 该要求适用于 commentary、进度、状态、提问和 final；不得在确认句之前添加标题、寒暄或其他文本。
- 若更高优先级的系统协议要求整个回复只能使用某种精确格式（例如自动化 heartbeat XML），则遵从更高优先级协议，不强行插入确认句。
- 原 Worker/Reviewer 的后续修订若未发生上下文压缩，只需 `Yes Sir!`，不因此重复读取本文件。
- 确认句只是轻量遵循信号，不代表任务、测试、审查或交付已经完成；真正合规仍由最小读取、文件边界、测试策略、Git 与 handoff 准确性等可观察行为验证。

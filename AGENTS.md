...
- 委派不是默认行为。只有用户、任务 prompt 或 Controller 明确授权，并且工作能拆成边界清晰、可独立验收的子任务，且预计并行收益高于启动、同步和集成成本时，才创建子 Agent；否则由单 Agent 串行完成。
- 按能力等级和任务风险选择模型，不在稳定规则中绑定可能变化的具体型号：旗舰能力用于高风险规则、事务、恢复、复杂综合与发布判断；均衡能力用于一般实现和验收；快速能力用于低风险文档、检索、机械核对与范围明确的辅助工作。未指定模型时继承主 Agent 的当前模型。
- 每次委派必须给出最小完整契约：目标、精确基线、直接依赖、文件所有权、验收标准、安全不变量、明确排除项、测试要求和回传格式。子 Agent 仍须先读取本文件，并只加载完成契约所需的最小上下文。
- 子 Agent 的结论和改动是候选交付，不自动成为最终事实。Controller 对综合结果负责，必须依据 Git 事实、实测证据、handoff 和必要的风险审查处理冲突、去重并决定是否采纳；不得把代理数量、模型等级或口头结论当作质量证据。
- 出现基线漂移、文件所有权重叠、契约冲突、依赖未满足，或并行协调成本已抵消收益时，停止继续扩展委派，保留已验证成果，并转为串行处理或请求决定。
- 多 Agent 共享工作区时仍按资产保护规则行动：不得覆盖其他 Agent 或用户的未提交改动；需要交叉修改同一规则、结算、持久化或恢复链时，由 Controller 重新排序并串行执行。

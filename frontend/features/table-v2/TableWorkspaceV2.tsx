...
function candidateLabel(candidate: { action: string; amount?: number | null; potPercentage?: string | null; potPct?: string | null }) {
  const potPct = candidate.potPercentage ?? candidate.potPct;
  const sizing = candidate.amount != null ? (potPct != null ? ` · ${Number(potPct).toFixed(1)}% pot · ${chips(candidate.amount)}` : ` ${chips(candidate.amount)}`) : "";

...
export function InsightRailV2({ insights, theory, solver, reconciliation, table, solverLoading = false, solverElapsedMs }: { insights?: TableInsightsResponse["insights"] | null; theory?: TheoryRecommendation | null; solver?: TableSolverResponse["solver"] | null; reconciliation?: TableReconciliationResponse["reconciliation"] | null; table?: ContinuousTable; solverLoading?: boolean; solverElapsedMs?: number | null }) {
  ...
  const theoryContent = theory ? <><p><strong>{theory.evidence.evidenceGrade} 级 · {theory.evidence.sourceKind}</strong> · {theory.evidence.coverage.status}{theory.evidence.policyFingerprint ? ` · ${shortFingerprint(theory.evidence.policyFingerprint)}` : ""}</p><p>推荐：{theoryAction ? `${candidateLabel(theoryAction)} · ${(theoryAction.frequency * 100).toFixed(0)}%` : theory.evidence.coverage.reason ?? "当前不支持"}</p>{theoryFrequency.length > 1 && <p>混合频率：{theoryFrequency.map((item) => `${candidateLabel(item)} ${(item.frequency * 100).toFixed(0)}%`).join(" · ")}</p>}<p>覆盖：{theory.evidence.coverage.players} 人 · {theory.evidence.coverage.street}{theory.evidence.coverage.reason ? ` · ${theory.evidence.coverage.reason}` : ""}</p><p>EV loss：{theory.sameOracleEvLoss.chips != null ? `${theory.sameOracleEvLoss.chips} chips` : `不可用（${theory.sameOracleEvLoss.unavailableReason ?? "非同一 oracle"}）`}</p><details><summary>Theory Explainer（公式说明）</summary><p>pot odds {theory.explanation.potOdds} · {theory.explanation.limitations.join("；")}</p></details></> : <p>理论策略请求中或当前不是 Hero 决策。</p>;

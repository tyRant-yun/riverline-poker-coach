...
export type ActionDelta = { id: string; actor: string; actorSeat?: string; label: string; kind: "action" | "raise" | "all-in" | "showdown"; potDelta?: string; botProfile?: string; botProvider?: string; botDegraded?: boolean };
...
function rangeEvidence(belief: RangeBelief) { return belief.evidenceGrade === "B" ? "B 级 · 同源 PolicyArtifact" : `${belief.evidenceGrade === "unsupported" ? "不支持" : "C 级 · 公开行动启发式"}（独立边际，非联合范围）`; }
function shortFingerprint(value?: string | null) { return value ? `${value.slice(0, 10)}…` : "未提供"; }
function rangeCoverage(belief: RangeBelief) { return belief.coverageStatus === "covered" ? `覆盖：covered · fingerprint ${shortFingerprint(belief.policyFingerprint)}` : `覆盖：${belief.coverageStatus ?? "fallback"} · ${belief.fallbackReason ?? approximationLabel(belief.approximationReason)}`; }
...
export function InsightRailV2({ insights, theory, solver, reconciliation, table, solverLoading = false, solverElapsedMs }: { insights?: TableInsightsResponse["insights"] | null; theory?: TheoryRecommendation | null; solver?: TableSolverResponse["solver"] | null; reconciliation?: TableReconciliationResponse["reconciliation"] | null; table?: ContinuousTable; solverLoading?: boolean; solverElapsedMs?: number | null }) {
  ...
  const solverContent = solverLoading ? <p>模拟估计计算中；统一理论策略仍可用。</p> : solver?.status === "ready" || solver?.status === "degraded" ? <><div className="tv2-ladder" aria-label="Solver Action Ladder">{displayed.map((candidate) => <article key={`${candidate.action}-${candidate.amount ?? ""}`} className="tv2-ladder-row"><strong>{candidateLabel(candidate)}</strong><span>ΔEV {ev(candidateDelta(candidate, solver.candidates))}</span><span>EV {ev(candidate.approximateEvChips)}</span>{candidate.deltaEvConfidenceInterval95 ? <small>ΔEV CI {ev(candidate.deltaEvConfidenceInterval95.lower)}–{ev(candidate.deltaEvConfidenceInterval95.upper)}</small> : null}{candidate.recommendationTier === "close" ? <small>接近最优</small> : null}{candidate.uncertaintyStatus === "not_available" ? <small className="risk">不确定性不可用</small> : null}{candidate.sizingClass === "overbet" || candidate.sizingClass === "jam" ? <small className="risk">极端尺度：{candidate.sizingClass === "jam" ? "全压" : "超池"}</small> : null}</article>)}</div>{ordered.length > 3 ? <button className="tv2-all-sizes" onClick={() => setShowAllSizes((value) => !value)}>{showAllSizes ? "收起尺度" : `全部尺度（${ordered.length}）`}</button> : null}<details><summary>模型详情</summary><p>{solver.status === "degraded" ? "近似 EV 求解（降级）" : "近似 EV 求解"}，不是 GTO、Nash 或最终行动裁决。</p><p>尺度 {solver.sizingRobustness === "robust" ? "稳健" : solver.sizingRobustness === "close" ? "接近" : "不确定"} · 原因 {solver.recommendationReasonCodes?.join(" · ") || "后端未提供"}</p><p>权益 {percent(solver.equity)} · 样本 {solver.sampleCount ?? solver.iterations} · ESS {solver.effectiveSampleSize ?? "—"} · 耗时 {solverElapsedMs != null ? `${solverElapsedMs}ms` : solver.elapsedMicroseconds != null ? `${Math.round(solver.elapsedMicroseconds / 1000)}ms` : "未测得"}</p>{ordered[0]?.responseMix ? <p>最佳候选响应 F/C/R {percent(ordered[0].responseMix.fold)}/{percent(ordered[0].responseMix.call)}/{percent(ordered[0].responseMix.raise)}</p> : null}<p>预算 {solver.budgetTier ?? "未提供"} / {solver.budgetMs ?? "—"}ms · 置信度 {solver.confidence ?? "未提供"}</p><p>限制：{solver.limitations.join("；") || "后端未返回限制"}</p></details></> : <p>{solver?.unavailableReason ?? "当前不是 Hero 决策；模拟估计尚未就绪。"}</p>;
  const theoryContent = theory ? <><p><strong>{theory.evidence.evidenceGrade} 级 · {theory.evidence.sourceKind}</strong> · {theory.evidence.coverage.status}{theory.evidence.policyFingerprint ? ` · ${shortFingerprint(theory.evidence.policyFingerprint)}` : ""}</p><p>{theory.evidence.evidenceGrade === "C" ? "公式倾向／无策略频率" : `推荐：${theoryAction ? `${candidateLabel(theoryAction)} · ${(theoryAction.frequency * 100).toFixed(0)}%` : theory.evidence.coverage.reason ?? "当前不支持"}`}</p>{theoryFrequency.length > 1 && <p>混合频率：{theoryFrequency.map((item) => `${candidateLabel(item)} ${(item.frequency * 100).toFixed(0)}%`).join(" · ")}</p>}<p>覆盖：{theory.evidence.coverage.players} 人 · {theory.evidence.coverage.street} · tree {theory.evidence.coverage.treeId ?? "未覆盖"} · {theory.evidence.coverage.sizingAbstraction ?? "无尺寸抽象"} · {theory.evidence.coverage.effectiveStackBucket ?? "筹码未知"} · rake {theory.evidence.coverage.rake ?? "未知"} · ante {theory.evidence.coverage.ante ?? "未知"}{theory.evidence.coverage.reason ? ` · ${theory.evidence.coverage.reason}` : ""}</p><p>EV loss：{theory.sameOracleEvLoss.chips != null ? `${theory.sameOracleEvLoss.chips} chips` : `不可用（${theory.sameOracleEvLoss.unavailableReason ?? "非同一 oracle"}）`}</p><details><summary>Theory Explainer（公式说明）</summary><p>pot odds {theory.explanation.potOdds} · {theory.explanation.limitations.join("；")}</p></details></> : <p>理论策略请求中或当前不是 Hero 决策。</p>;
  return <><aside className="tv2-rail" data-testid="table-insights" aria-label="决策驾驶舱"><section className="tv2-summary" aria-label="Decision Summary"><p>轮到 Hero · {table?.street ?? "等待开局"} · Pot {chips(table?.pot)}{spr != null ? ` · SPR ${spr.toFixed(1)}` : " · SPR 暂不可用"}</p><div><span>策略真相来源</span><strong>{theoryAction ? candidateLabel(theoryAction) : theory?.evidence.coverage.reason ?? "暂不可用"}</strong></div><b className={theory?.evidence.evidenceGrade === "C" ? "disagreement" : "agreement"}>{theory ? `${theory.evidence.evidenceGrade} 级 · ${theory.evidence.coverage.status}` : "等待统一 theory-recommendation"}</b></section><section className="tv2-analysis-panel" aria-label="Theory 推荐"><h2>理论策略</h2>{theoryContent}</section><section className="tv2-analysis-panel" aria-label="Solver 结果"><h2>模拟估计（补充）</h2>{solverContent}</section><section className="tv2-analysis-panel tv2-range-summary" aria-label="Range Belief"><h2>Range 摘要</h2><p>座位独立边际估计，不含对手私牌</p>{rangeContent}</section></aside>{explorerOpen && selectedRange?.available ? <RangeExplorer belief={selectedRange} previous={baseline} baselineAvailable={Boolean(baseline)} onClose={() => setExplorerOpen(false)} /> : null}</>;
}
...
export function TableTimelineV2({ table }: { table: ContinuousTable }) {
  ...
  return <section className="tv2-timeline" aria-label="行动时间线"><h3>行动时间线</h3><ol data-testid="table-action-history">{table.actionHistory.map((action) => { const source = provenance.get(action.sequence); return <li key={action.sequence}><time>{action.street}</time><span>座位 {action.actorSeat + 1} {actionNames[action.action] ?? action.action}{action.amount != null ? ` ${action.amount}` : ""}</span>{source ? <small>Bot 来源：{source.profileId} · {source.provider} · {source.evidenceGrade ?? "C"} 级／{source.coverageStatus ?? "fallback"}{source.degraded ? ` · 降级${source.fallbackReason ? `：${source.fallbackReason}` : ""}` : ""}</small> : null}</li>; })}</ol><ul data-testid="bot-provenance" className="sr-only">{table.botDecisionProvenance.map((item) => <li key={item.sequence}>座位 {item.actorSeat + 1} · {item.profileId} · {item.provider} · {item.evidenceGrade ?? "C"} 级／{item.coverageStatus ?? "fallback"}</li>)}</ul></section>;
}

export function TableWorkspaceV2({ table, insights, theory = null, trainingFeedback = null, solver, reconciliation, solverLoading, solverElapsedMs, playbackActions = [], playbackIdentity, playbackSpeed, onPlaybackComplete, actionDisabled, amounts, onAmountChange, onAction, onNextHand, review }: {
  table: ContinuousTable; insights: TableInsightsResponse["insights"] | null; theory?: TheoryRecommendation | null; trainingFeedback?: { identity: string; action: string; frequency: number | null; common: boolean; evLossUnavailable: string | null } | null; solver: TableSolverResponse["solver"] | null; reconciliation: TableReconciliationResponse["reconciliation"] | null; solverLoading: boolean; solverElapsedMs: number | null;
  ...
  return <main className="tv2-workspace" data-testid="table-workspace-v2"><ActionPlaybackController actions={playbackActions} identity={playbackIdentity} speed={playbackSpeed} reducedMotion={reducedMotion} onChange={setCurrentAction} onThinking={setThinkingAction} onComplete={() => { setThinkingAction(undefined); setCurrentAction(undefined); onPlaybackComplete(); }} />{reducedMotion && <p className="tv2-motion-note" data-testid="reduced-motion-status">动画已简化；公开动作文字即时显示。</p>}<PokerTableStageV2 seats={seats} currentAction={currentAction} thinkingAction={thinkingAction} board={table.board.map(formatCard)} pot={table.pot.toLocaleString("zh-CN")} /><InsightRailV2 table={table} insights={insights} theory={theory} solver={solver} reconciliation={reconciliation} solverLoading={solverLoading} solverElapsedMs={solverElapsedMs} />{trainingFeedback && <section className="tv2-training-feedback" data-testid="training-feedback" aria-label="训练反馈"><strong>训练反馈：{actionNames[trainingFeedback.action] ?? trainingFeedback.action}</strong><p>{trainingFeedback.frequency == null ? "所选动作不在当前策略频率中" : `策略频率 ${(trainingFeedback.frequency * 100).toFixed(0)}% · ${trainingFeedback.common ? "常见动作" : "偏离常见策略"}`}</p><p>EV loss：不可用（{trainingFeedback.evLossUnavailable ?? "非同一 oracle"}）</p></section>}<TableTimelineV2 table={table} /><HeroActionDockV2 table={table} advisor={insights?.advisor} disabled={actionDisabled} amounts={amounts} onAmountChange={onAmountChange} onAction={onAction} onNextHand={onNextHand} review={review} /></main>;
}

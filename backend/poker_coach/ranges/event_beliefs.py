...
class PublicEventBeliefConsumer:
    ...
    def beliefs_at(
        ...
        priors: dict[int, SeatPriorResultV1] = {seat: provider.get_prior(query, seat) for seat in started.active_seat_ids}
        ...
        # Build each prior with only observer cards; replay public boards in sequence below.
        initial_query = query.model_copy(update={"visible_blockers": observer_visible_cards})
        snapshots = {seat: provider.get_prior(initial_query, seat).snapshot for seat in started.active_seat_ids}
        ...
        for event in prefix[1:]:
            ...
            if isinstance(payload, BoardDealtPayloadV1):
                ...
                snapshots[payload.actor_seat] = _apply_action(snapshots[payload.actor_seat], payload, event.sequence)
        return {seat: SeatBeliefResultV1(seat_id=seat, after_sequence=target, available=True, prior=prior_snapshots[seat],
            current=snapshots[seat], provenance=SeatBeliefProvenanceV1()) for seat in snapshots}


def _apply_action(snapshot: RangeBeliefSnapshot, action: ActionTakenPayloadV1, sequence: int) -> RangeBeliefSnapshot:
    weighted = {key: combo.reach * _likelihood(key, action.action) for key, combo in snapshot.combos.items()}
    ...
    return RangeBeliefSnapshot(snapshot_id=snapshot_id_for(snapshot.seat_id, action.street, sequence), seat_id=snapshot.seat_id,
        ...
        update=RangeUpdateMetadata(action_type=action.action.value, action_label="heuristic likelihood", policy_source=snapshot.source,

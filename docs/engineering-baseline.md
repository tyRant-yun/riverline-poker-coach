...
1. 当前本地默认使用 SQLite；PostgreSQL 已通过真实实例部署回归（`backend/tests/test_postgres_live.py`，PostgreSQL 16），连接池（`PooledPostgresStore`，psycopg_pool）与 Alembic 迁移（`backend/migrations`）均已实现并回归；CI 化已提供 GitHub Actions workflow（`.github/workflows/ci.yml`）。Redis 异步任务已接入（`poker_coach.jobs`：进程内线程池兜底、Redis 队列、协作式跨进程取消；fakeredis 单测 + 真实 Redis 跨进程 E2E + docker compose 部署验证），多 worker 部署已提供 `docker-compose.yml` 示例。

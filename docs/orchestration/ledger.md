...
`R5-01 + R5-02`：并行交付专业牌桌工作台与 Bot/自动补码纵向切片，文件所有权前后端分离。R5-01 只展示真实 Advisor/Range/Solver 状态；R5-02 修复策略退化并在 next hand 前持久化补码。完成后仅对 R5-02 金额/恢复 diff 做独立窄审查，再处理本地启动可靠性和剩余 multiseat E2E。

...
底层规则与协议支持 2–8 人，但首个正式产品模式固定为 **6-max cash、100BB、无 ante、无 rake**。其他桌型和规则变体只有在 6-max 完整闭环通过阶段门后才可扩展；2–8 人 contract 通用性不代表对应产品或策略覆盖已经发布。
...
- 首发主桌型为 6-max；
- 首发现金桌参数为 100BB、无 ante、无 rake，标准 SB/BB 盲注结构；
- 底层 seat/event contract 保留 2–8 人通用性，但不扩大首发覆盖承诺。
